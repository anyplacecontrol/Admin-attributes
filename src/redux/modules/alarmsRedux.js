...
export const alarmsInitialState = {
  ...
  columns: tableColumns.ALARMS_COLUMNS,
};

//*******************************************************************************

export default function reducer(state = alarmsInitialState, action = {}) {
  let result = BaseTableReducer(PREFIX, state, action, alarmsInitialState);
...
class AlarmsActions extends BaseTableActions {
  // ABSTRACT ACTIONS REALIZATION

  _deleteItem(alarmObj) {
    return async (dispatch, getState) => {
      //   await alarmsApi.deleteItem(alarmObj);
    };
  }
  ...
  _fetchItemsFromNetwork(
    ...
    return async (dispatch, getState) => {
      let fetchedResponse = await alarmsApi.getItems(
        filter,
        topRowNumber,
        itemsPerPage
        // sortBy,
      ...
      fetchedResponse.items.forEach((alarmObj, i) => {
        let machine = machines.find(machine => machine.id === alarmObj.MachineId);
  ...
  goto_editItem(itemId) {
    return async dispatch => {
      dispatch(routing.goto_EditItem(ROUTE_NAMES.alarmsView, itemId));
    };
  }
...
export const alarmsActions = new AlarmsActions();
