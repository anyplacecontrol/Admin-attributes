...
export const BaseTableInitialState = {
  maxItemsQty: 0, //quantity of orders in database
  ...
  switch (action.type) {
    ...
    case _ACTION_TYPE_PREFIX + BaseTableTypes.FETCH_ITEMS_COMPLETE: {
      return {
        ...
        maxItemsQty: action.maxItemsQty,
        checkedItems: []
      };
    }
...
export class BaseTableActions {
  ...
  fetchItems = (
    ...
    return async (dispatch, getState) => {
      ...
      if (!fetchedResponse.maxItemsQty) {
        fetchedResponse.maxItemsQty = fetchedResponse.items.length;
      }
      ...
      for (let i = 0; i < fetchedResponse.items.length; i++) {
        ...
        if (
          ...
          rowNumber = fetchedResponse.maxItemsQty - topRowNumber - i;
        } else {
      ...
      if (!isCvsExport) {
        await dispatch({
          ...
          maxItemsQty: fetchedResponse.maxItemsQty,
  ...
  getMaxItemsQty = globalState => {
    return this._getStateSlice(globalState).maxItemsQty;
  };
      ...
      try {
        ...
        if (!isBulkOperation) {
          ...
          // else 
          {
            ...
            await dispatch(this.fetchItems(topRowNumber, false, false, null, true)); //do not throw exception, but show standard alert
          }
        }        
    ...
    return async (dispatch, getState) => {
      ...
      if (!isAnyError && isBulkOperation) {
        if (func === this._deleteItem)
          ...
          let topRowNumber = this._getStateSlice(getState()).topRowNumber;
          await dispatch(this.fetchItems(topRowNumber, false, false, null, false));
        }
      }
