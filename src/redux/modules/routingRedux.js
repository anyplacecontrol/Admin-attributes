...
function getStatusViewUrl(statusKind) {
  switch (statusKind) {
    case statusesRedux.statusesProducts:
      return ROUTE_NAMES.statusesProductsView;
    case statusesRedux.statusesProductItems:
      return ROUTE_NAMES.statusesProductItemsView;
    case statusesRedux.statusesOrders:
      return ROUTE_NAMES.statusesOrdersView;
    case statusesRedux.statusesStores:
      return ROUTE_NAMES.statusesStoresView;
    case statusesRedux.statusesMachines:
      return ROUTE_NAMES.statusesMachinesView;
    case statusesRedux.statusesKiosks:
      return ROUTE_NAMES.statusesKiosksView;
    case statusesRedux.statusesCustomers:
      return ROUTE_NAMES.statusesCustomersView;
    default:
      throw "Unknown statusKind in getStatusViewUrl";
  }
}

// Status
// export function goto_AddStatus(statusKind) {
//   return async function(dispatch) {
//     let url = getStatusViewUrl(statusKind);
//     dispatch(goto_Page(url));
//   };
// }

export function goto_EditStatus(itemId, statusKind) {
  return async function(dispatch) {
    let url = getStatusViewUrl(statusKind);
    dispatch(goto_Page(url + "?" + QUERY_PARAMS.itemId + "=" + itemId));
  };
}
