...
export class BaseView_ extends React.Component {
  ...
  render() {
    ...
    let caption =  this.props.actionsProvider ?  "Add New " + this.props.viewName : this.props.viewName;

    if (this.props.actionsProvider && this.props.stateSlice) {
      if (
        !this.props.actionsProvider._isNewItem(
          this.props.stateSlice          
        )
      ) {
        if (isSubmit) caption = "Edit Existing " + this.props.viewName;
        else caption = "View Existing " + this.props.viewName;
      }
    }
